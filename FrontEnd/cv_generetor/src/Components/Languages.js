...
class Languages extends React.Component{
  ...
  render(){
    ...
    return (
      <>
      <h4>Languages <button onClick={this.openBox} className='btn'><GrAddCircle/></button></h4>
      
	  <>
		  {language.map((output,id) =>(
		...
		<Dialog open={this.state.open} onClose={this.closeBox}>
			<DialogTitle>Add languages<button style={{float:'right'}} type='button' className="btn btn-outline-danger btn-sm" onClick={this.closeBox}><GrClose/></button></DialogTitle>
			<DialogContent>
				<DialogContentText>
					Fill this form
				</DialogContentText>
				<form onSubmit={this.handleSubmit}>
					<input type='text' ref={input => this.lge = input} className='form-control' placeholder='Language' /><br/>
